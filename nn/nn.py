...
class FaceGenerator:
    ...
    def get_training_data(self, datafolder):
        ...
        training_data = np.reshape(
            training_data, (-1, self.image_width, self.image_height, self.channels))
    ...
    def build_generator(self):
        ...
        model.add(Dense(256*4*4, activation="relu",
                        input_dim=self.random_noise_dimension))
    ...
    def build_discriminator(self):
        ...
        model.add(Conv2D(32, kernel_size=3, strides=2,
                         input_shape=self.image_shape, padding="same"))
    ...
    def train(self, datafolder, epochs, batch_size, save_images_interval):
        ...
        for epoch in range(epochs):
            ...
            random_noise = np.random.normal(
                0, 1, (batch_size, self.random_noise_dimension))
            ...
            discriminator_loss = 0.5 * \
                np.add(discriminator_loss_real, discriminator_loss_generated)
    ...
    def save_images(self, epoch):
        ...
        noise = np.random.normal(
            0, 1, (rows * columns, self.random_noise_dimension))
