				...
				int dx, dy;
				{
					...
					if (releasePoint != null) {
						...
						switch (birdOnSling) {
							...
							case WhiteBird:  tapInterval = 70 /*+ random.nextInt(20)*/; break;
							case BlackBird:  tapInterval = 70 /*+ random.nextInt(20)*/; break;
			...
			if (executedShots + 1 <= executedShotsList.size()) {
				...
				if (chosenTargetObject.equals(previousTargetObject)) {
					...
					if (executedShotsList.size() == 1) {
						CustomLogger.info("[Meta] Only shot in the List. shot was rated as optimal shot.");
